...
qx.Class.define("rhyacotriton.ToolBar",
{
  ...
  */
  ...
  construct : function(controller)
  {
    ...
    this.__reconnectBtn = new qx.ui.toolbar.Button(this.tr("Reconnect"), "icon/22/devices/network-wired.png");
    this.__reconnectBtn.setCommand(controller.getCommand("reconnect"));
    this.add(this.__reconnectBtn);

    this.__reloadBtn = new qx.ui.toolbar.Button(this.tr("Refresh"), "icon/22/actions/view-refresh.png");
    this.__reloadBtn.setCommand(controller.getCommand("reload"));
    this.add(this.__reloadBtn);

    // Add/Remove buttons
    this.__addBtn = new qx.ui.toolbar.Button(this.tr("Add"), "icon/22/actions/list-add.png");
    this.__addBtn.setCommand(controller.getCommand("addTorrent"));
    this.add(this.__addBtn);

    this.__removeBtn = new qx.ui.toolbar.Button(this.tr("Remove"), "icon/22/actions/list-remove.png");
    this.__removeBtn.setCommand(controller.getCommand("removeSelectedRows"));
    this.add(this.__removeBtn);

    this.__startBtn = new qx.ui.toolbar.Button(this.tr("Start"), "icon/22/actions/media-playback-start.png");
    this.__startBtn.setCommand(controller.getCommand("startSelectedRows"));
    this.add(this.__startBtn);

    this.__stopBtn = new qx.ui.toolbar.Button(this.tr("Pause"), "icon/22/actions/media-playback-pause.png");
    this.__stopBtn.setCommand(controller.getCommand("stopSelectedRows"));
    this.add(this.__stopBtn);

    this.addSpacer();

    this.__peersView = new qx.ui.toolbar.RadioButton(this.tr("Peers"), "icon/22/apps/preferences-users.png");
    this.__peersView.setUserData("value", "peers");
    this.add(this.__peersView);

    this.__filesView = new qx.ui.toolbar.RadioButton(this.tr("Files"), "icon/22/places/folder-open.png");
    this.__filesView.setUserData("value", "files");
    this.add(this.__filesView);

    this.__logView = new qx.ui.toolbar.RadioButton(this.tr("Log"), "icon/22/apps/utilities-log-viewer.png");
    ...
    this.__viewGroup.addListener("changeSelection", controller.syncStackView, controller);
  ...
  members :
  {
    ...
    {
      [ this.__removeBtn, this.__startBtn, this.__stopBtn ].map(function(x) {
        x.setEnabled(flag);
      });
    },
    ...
    setEnabled : function(flag)
    {
      ...
      if (flag == this.__isRowButtonsEnabled) this.__enableRowButtons(flag);
    }
  }
});